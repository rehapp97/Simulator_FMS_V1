...
def Operation_sequencing_EDD(con2, Job):
    LstOP = []
        ...
        if (Job.P[i] != None):
            for j in range(Job.P[i].PT.i_Num_Operation):
                if (Job.P[i].PT.i_Num_Alternative_Machine[j] != 0):
                    LstOP.append([Job.P[i].i_Part_index, Job.P[i], j, Job.P[i].i_Due_date, sum(Job.P[i].PT.i_Standard_Time[j])/len(Job.P[i].PT.i_Standard_Time[j]), min(Job.P[i].PT.i_Standard_Time[j])])

    LstOP = sorted(LstOP, key=itemgetter(3, 2, 4, 5))

    for i in range(Job.i_Total_Number_Operation):
        Job.Part_Route[i] = LstOP[i][1]
        Job.i_OP_Route[i] = LstOP[i][2]

    l = 0
    return Job

def Operation_sequencing_MMST(con2, Job): #modified minimum slack time
    LstPart = []
    Job.Part_Route = [None for i in range(Job.i_Total_Number_Operation+1)]
    Job.i_OP_Route = [None for i in range(Job.i_Total_Number_Operation+1)]
    ...
    for i in range(Job.Pallet_job.i_Num_Fixt):
        ...
        if (Job.P[i] != None):
            for j in range(Job.P[i].PT.i_Num_Operation):
                if (Job.P[i].PT.i_Num_Alternative_Machine[j] != 0):
                    f_Remain_work += sum(Job.P[i].PT.i_Standard_Time[j])/len(Job.P[i].PT.i_Standard_Time[j])
            for j in range(Job.P[i].PT.i_Num_Operation):
                if (Job.P[i].PT.i_Num_Alternative_Machine[j] != 0):
                    i_Operation_Idx = j
                    f_Avg_proc = sum(Job.P[i].PT.i_Standard_Time[j])/len(Job.P[i].PT.i_Standard_Time[j])
                    f_Slack = (Job.P[i].i_Due_date - f_Avg_proc) / f_Remain_work
                    LstOp.append([Job.P[i].i_Part_index, Job.P[i].PT.i_Fixture_Type, i_Operation_Idx, Job.P[i], f_Slack]) #Operation간 마지막 값(f_Slack)만 비교한다.
                    f_Remain_work -= f_Avg_proc
    ...
    for i in range(Job.i_Total_Number_Operation):
        ...
        for j in range(len(LstPart)):
            Temp_op = LstPart[j][0] #Loading된 각 part의 첫 번째 operation, Job.i_OP_Route와 Job.Part_Route에 들어간 operation은 삭제하기 때문에 계속해서 첫 번째꺼만 체크하는 것이다.
            if (Temp_op[4] < min0): #각 operation의 modified slack time끼리 비교한다.
                min0 = Temp_op[4]
                min0_idx = Temp_op[2]
                min0_part_idx = j
        Job.i_OP_Route[i] = LstPart[min0_part_idx][0][2] #Loading된 각 part의 첫 operation 중에 작은값에 해당하는 operation을 추가한다.
...
def Operation_seqeuncing_option(Operation_Sequence_Option, con2, Job):
    if (Operation_Sequence_Option == 1):
        Job = Operation_sequencing_EDD(con2, Job)
    elif (Operation_Sequence_Option == 2):
        Job = Operation_sequencing_MMST(con2, Job)
    elif (Operation_Sequence_Option == 3):
        Job = Operation_sequencing_SMPT(con2, Job)
    elif (Operation_Sequence_Option == 4):
...
def Dispatching_EPADD(M, Job_queue, Time):
    min0 = 99999999
    Temp = Job_queue.head.next
    Temp_job = None
    i_Next_rest_time = M.i_Next_Rest_Time
    ...
    while (Temp != Job_queue.tail):
        if (Temp.Obj.i_current_state==0):
            if ((Temp.Obj.i_Processing_time[Temp.Obj.i_current_position]+Time <= i_Next_rest_time)and(Temp.Obj.Machine_Route[Temp.Obj.i_current_position]==M)and(Temp.Obj.i_Terminate!=1)):
                if (min0 > (Temp.Obj.f_avg_duedate)/(Temp.Obj.i_Num_parts)):
                    Temp_job = Temp.Obj
                    min0 = (Temp.Obj.f_avg_duedate) / (Temp.Obj.i_Num_parts)
        Temp = Temp.next
...
def Dispatching_EPMDD(M, Job_queue, Time):
    min0 = 99999999
    Temp = Job_queue.head.next
    Temp_job = None
    i_Next_rest_time = M.i_Next_Rest_Time
    ...
    while (Temp != Job_queue.tail):
        if (Temp.Obj.i_current_state==0):
            LstDue = []
            for i in range(Temp.Obj.Pallet_job.i_Num_Fixt):
                if(Temp.Obj.P[i] != None):
                    LstDue.append(Temp.Obj.P[i].i_Due_date)
            if ((Temp.Obj.i_Processing_time[Temp.Obj.i_current_position]+Time <= i_Next_rest_time)and(Temp.Obj.Machine_Route[Temp.Obj.i_current_position]==M)and(Temp.Obj.i_Terminate!=1)):
                if (min0 > min(LstDue)):
                    Temp_job = Temp.Obj
                    min0 = min(LstDue)
...
def Dispatching_CR(M, Job_queue, Time):
    min0 = 99999999
    Temp = Job_queue.head.next
    Temp_job = None
    i_Next_rest_time = M.i_Next_Rest_Time
    ...
    while (Temp != Job_queue.tail):
        TWKR = TWKR0(Temp.Obj)
        Temp.Obj.i_TWKR = TWKR
        if (Temp.Obj.i_current_state==0):
            due = (Temp.Obj.f_avg_duedate) / (Temp.Obj.i_Num_parts)
...
def Dispatching_MDD(M, Job_queue, Time):
    ...
    while (Temp != Job_queue.tail):
        ...
        if (Temp.Obj.i_current_state==0):
            ...
            if ((Temp.Obj.i_Processing_time[Temp.Obj.i_current_position]+Time <= i_Next_rest_time)and(Temp.Obj.i_current_state==0)and(Temp.Obj.Machine_Route[Temp.Obj.i_current_position]==M)and(Temp.Obj.i_Terminate!=1)):
                if (min0 > max0(due, Time+TWKR)):
                    Temp_job = Temp.Obj
                    min0 = max0(due, Time+TWKR)
        Temp = Temp.next
...
def Dispatching_option_selection(Dispatching_Option, Mac_Temp, Job_Queue, Time):
    if (Dispatching_Option == 1):
        ...
        Job_temp = Dispatching_EPADD(Mac_Temp, Job_Queue, Time) #earliest pallet average due-date
    elif (Dispatching_Option == 3):
        Job_temp = Dispatching_EPMDD(Mac_Temp, Job_Queue, Time) #earliest pallet maximum due-date
    elif (Dispatching_Option == 4):
        Job_temp = Dispatching_CR(Mac_Temp, Job_Queue, Time) #critical ratio
    elif (Dispatching_Option == 5):
        Job_temp = Dispatching_MDD(Mac_Temp, Job_Queue, Time) #modified due-date
    else:
...
def Main():
    finName = 'Instance_5_15_40_4_0_03.txt'
    #finName = sys.argv[5]
    fin = open(finName, 'r')
