...
class Config_T():
    ...
    def initialize_Setting(self, fin):
        ...
        for i in range(self.i_Num_Parts): #[index, i_Due_date, i_All_processing_time]
                LstP.append([self.P[i].i_Part_index, self.P[i].i_All_processing_time/sum(self.P[i].PT.i_Num_Alternative_Machine), self.P[i].i_All_processing_time, LstSum_min_proc[i], self.P[i]])
        if (self.Input_Sequence_Option == 1):
            LstP = sorted(LstP, key=itemgetter(1, 2))
        else:
