...
def ExtAttribute0(con2, Job):
    LstColumn = np.array([])
    for i in len(Job.Part_Route):
        if (i != len(Job.Part_Route)):
            Secq = i
            Prec = Job.i_OP_Route[i]
            for j in range(Job.Part_Route[i].PT.i_Num_Operation):
                if (j != 0) and (j != Job.Part_Route[i].PT.i_Num_Operation+1):
                    MinPt = min(Job.Part_Route[i].PT.i_Standard_Time[j])
                    AvgPt = sum(Job.Part_Route[i].PT.i_Standard_Time[j]) / len(Job.Part_Route[i].PT.i_Standard_Time[j])
                    Num_AlterMac = len(Job.Part_Route[i].PT.i_Standard_Time[j])
    ...
    for i in range(con1.i_Num_pallet):
        if (con1.Pallet_type[i].i_rest_Pallet == 1):
            ...
            if (Temp_job.i_Empty == 0): #pallet에 part들이 올라가는것이 없으면, 1
                ...
                LLBuffer.push_back(con1.J[con1.Job_index]) #operations sequence와 initial machine이 정해진 pallet
